...
- **Mixed loadouts.**
  The per-soldier `weapon_id` and `shield_id` arrays could carry a mixed regiment, but `combat_profile()`, `shield_rest_angle()`, and the type-level stats read per unit;
