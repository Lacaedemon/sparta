...
Status: **historical design note -- all four slices shipped.**
Tracks
...
(verified 2026-09-19).
The "Suggested phasing" section marks slice 2 as
...
- Does `retire_current_order`'s existing MOVE/ATTACK promotion special-casing
  ...
  **Exercised by the `COMBO` composite** (`Unit.begin_combo`,
  `orders-queue-design.md`'s "Macro expansion"): its children are exactly such
