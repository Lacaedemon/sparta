...
```sh
# 1. The run's transcripts (hash streams, full state dumps, the raw analyzer JSON,
#    the report itself), kept 90 days; the tracking issue names the run and artifact.
gh run download <run-id> -R Lacaedemon/sparta -n website-demo-defect-sweep-<run-id> -D /tmp/tx-ci
cat /tmp/tx-ci/platform.txt            # os=Linux, the Godot version, the tick cadence
