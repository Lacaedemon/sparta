...
# --- bounding a scan to the attributable window ---------------------------------

func test_snapshots_up_to_bounds_inclusively_and_keeps_input_order() -> void:
	var slots: Array = _grid(6, 4, SPACING)
	var series: Array = [
		_snapshot(0, slots.duplicate(), slots),
		_snapshot(60, slots.duplicate(), slots),
		_snapshot(120, slots.duplicate(), slots)]
	var bounded: Array = DemoDefects.snapshots_up_to(series, 60)
	assert_eq(bounded.size(), 2, "the bound is inclusive of its own tick")
	assert_eq(int(bounded[0]["tick"]), 0, "input order is preserved")
	assert_eq(int(bounded[1]["tick"]), 60, "and the bounding tick is the last kept")


func test_snapshots_up_to_is_unbounded_for_a_negative_tick() -> void:
	var slots: Array = _grid(6, 4, SPACING)
	var series: Array = [_snapshot(0, slots.duplicate(), slots), _snapshot(60, slots.duplicate(), slots)]
	assert_eq(DemoDefects.snapshots_up_to(series, -1).size(), 2,
			"a negative bound means no bound, so a clip with no known divergence scans in full")


func test_snapshots_up_to_can_empty_the_series_rather_than_judging_vacuously() -> void:
	var slots: Array = _grid(6, 4, SPACING)
	var series: Array = [_snapshot(60, slots.duplicate(), slots), _snapshot(120, slots.duplicate(), slots)]
	assert_eq(DemoDefects.snapshots_up_to(series, 30).size(), 0,
			"a window too narrow to hold a snapshot returns nothing, so the caller reports "
			+ "'nothing to judge' instead of a clean scan of zero samples")


func test_bounding_to_the_pre_divergence_window_excludes_a_later_defect() -> void:
	# The whole point of the window: a defect that only develops after two runs have
	# diverged must not be attributed to the change under review. Same series, two
	# bounds -- the full range fails, the pre-divergence prefix passes.
	var slots: Array = _grid(6, 4, SPACING)
	var blob: Array = _grid(6, 4, SPACING * 0.35)
	var series: Array = [
		_snapshot(0, slots.duplicate(), slots),
		_snapshot(60, slots.duplicate(), slots),
		_snapshot(120, slots.duplicate(), slots),
		_snapshot(180, blob, slots),
		_snapshot(240, blob, slots),
		_snapshot(300, blob, slots)]
	assert_false(bool(_verdict(DemoDefects.analyze(series), "blob")["pass"]),
			"scanned in full, the late compression fails the blob verdict")
	var window: Array = DemoDefects.snapshots_up_to(series, 120)
	assert_true(bool(_verdict(DemoDefects.analyze(window), "blob")["pass"]),
			"bounded to the ticks before it, the same series is clean")
