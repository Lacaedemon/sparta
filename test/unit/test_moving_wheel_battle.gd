...
func test_moving_wheel_hinge_holds_pace_while_outer_file_runs_faster_then_settles() -> void:
	...
	# formed_turn_tracking_frac derates how fast a formed march's own centre-pivot corrects
	# ordinary spawn/coupling settling noise (Unit._move_to's pivot_as_formation), so a bit
	# more of that noise now survives into the wheel-arm moment than the pre-derate 1.5x
	# margin was sized against -- a hair more residual offset for whichever soldier lands
	# nearest the wheel's hinge, not the wheel's own kinematics running hot. 1.7x keeps real
	# headroom above the measured worst case post-derate (~51.6 wu/s against a 34 wu/s cruise).
