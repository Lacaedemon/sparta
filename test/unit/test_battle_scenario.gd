...
func test_scenario_spawns_exactly_its_units_with_types_positions_and_overrides() -> void:
	...
	assert_almost_eq(spear.training, 0.2, 0.001,
		"the training override wins over Spearmen's own type default (0.75)")
	...
	for horse: Unit in team1:
		...
		assert_false(horse.file_major_reform,
			"with no override, Cavalry's own file_major_reform default (off/structureless) applies")
