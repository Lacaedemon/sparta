...
func test_every_name_produces_a_valid_wav() -> void:
	...
	for name in SfxScript.NAMES:
		var wav: AudioStreamWAV = synth.synth(name)
		assert_not_null(wav, "%s synthesises a stream" % name)
		assert_eq(wav.format, AudioStreamWAV.FORMAT_16_BITS, "%s is 16-bit" % name)
		assert_eq(wav.mix_rate, 22050, "%s uses the synth mix rate" % name)
		assert_false(wav.stereo, "%s is mono" % name)
		assert_true(wav.data.size() > 0, "%s has samples" % name)
		assert_eq(wav.data.size() % 2, 0, "%s is a whole number of 16-bit samples" % name)
