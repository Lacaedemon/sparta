...
func test_deep_turning_march_uses_wide_proximity_for_direct_arrival() -> void:
	...
	var target_vec: Vector2 = SoldierBodies._corridor_to_slot(deep_unit, 0, slot_pos, deep_unit.soldiers)
	var expected_direct: Vector2 = slot_pos - soldier_pos
...
func test_shallow_turning_march_keeps_narrow_proximity() -> void:
	...
	# the shape of the regression an unconditional (turning-alone) widening produced on
	# ordinary shallow formations: coast_to_stop's settle timing and rout_rally's
	# overlap/shape_residual readings both moved on shallow blocks with no deep formation
	# involved, while the parade_ground/showcase slowdown -- the intended cost of derating an
	# actually deep block -- is untouched by this depth gate either way.
	...
	var target_vec: Vector2 = SoldierBodies._corridor_to_slot(shallow_unit, 0, slot_pos, shallow_unit.soldiers)
