...
func test_near_front_soldier_indices_and_position_anchor_indices_work_while_merely_in_contact() -> void:
	...
	# the anchor dilutes over the whole, mostly-static block the same way #783/#784 fixed for
	# a fighting regiment's charge (see docs/individual-collision-design.md). Both functions
	# must return the identical, nonempty selection they would if truly engaged.
	var u := _make_unit(120)
	u.seed_sim_soldiers()
	u._in_enemy_contact = true
