...
func test_landing_is_deterministic() -> void:
	...
	for _run in range(2):
		...
		var fb: Array = _field_and_battle(shooter, target)
		var field: ProjectileField = fb[0]
		field.launch(shooter.position, target.position, shooter.uid, target.uid, 6, 1.0, ProjectilePhysics.ANGLE_ARCED)
...
func test_fallback_applies_the_flanked_count_without_re_flanking() -> void:
	...
	field.launch(shooter.position, target.position, shooter.uid, target.uid, 5, 2.0, ProjectilePhysics.ANGLE_ARCED)
	field.step(10.0, fb[1])
...
func test_zero_distance_shot_still_lands() -> void:
	...
	field.launch(shooter.position, target.position, shooter.uid, target.uid, 3, 1.0, ProjectilePhysics.ANGLE_ARCED)
	field.step(1.0, fb[1])
...
func test_facing_the_archers_beats_turning_your_back_at_the_default_odds() -> void:
	...
	front[0].launch(front[2].position, front[3].position, front[2].uid, front[3].uid, 10, 1.0, ProjectilePhysics.ANGLE_ARCED)
	front[0].step(10.0, front[1])
	var front_losses: int = 24 - front[3].soldiers
	Replay.rng.seed = SEED
	var back: Array = _shot_from_the_north(Vector2.DOWN)
	back[0].launch(back[2].position, back[3].position, back[2].uid, back[3].uid, 10, 1.0, ProjectilePhysics.ANGLE_ARCED)
...
func test_zeroing_the_block_scale_restores_the_unshielded_resolution() -> void:
	...
	var field: ProjectileField = s[0]
	field.shield_block_scale = 0.0
	field.launch(s[2].position, s[3].position, s[2].uid, s[3].uid, 8, 1.0, ProjectilePhysics.ANGLE_ARCED)
	field.step(10.0, s[1])
