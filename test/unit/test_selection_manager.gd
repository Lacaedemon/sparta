...
func test_flag_pick_distance_hits_the_standard_and_misses_the_body_and_empty_space() -> void:
	...
	assert_almost_eq(sm._flag_pick_distance_sq(u, flag_world), 0.0, 0.001,
			"a cursor on the standard's centre is zero distance from it")
	assert_eq(sm._flag_pick_distance_sq(u, u.global_position), -1.0,
			"the body centre is well below the raised standard, so not a flag hit")
	assert_eq(sm._flag_pick_distance_sq(u, u.global_position + Vector2(9999, 0)), -1.0,
...
func test_flag_pick_tracks_the_shifted_standard() -> void:
	...
	assert_gte(sm._flag_pick_distance_sq(u, hit_at), 0.0,
	...
	assert_eq(sm._flag_pick_distance_sq(u, stale_spot), -1.0,
