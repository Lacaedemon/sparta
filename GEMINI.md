...
2. **Proactive corrective actions & PR workflow**: Whenever a gap, missing PR, or workflow instruction is identified, or when a capability/memory instruction is given:
   ...
   - **Auto-merge under `mwc`**: If `mwc` is active for the session, once CI passes AND the code review comment payload is verified clean (its review-data block says `"verdict": "CLEAN"` and its prose verdict reads **Ready for merge**, and never merge over **Needs more work** even if the check-run job status passed), execute the merge and post-merge wrap-up immediately without stopping.
