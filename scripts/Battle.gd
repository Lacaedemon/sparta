...
func _spawn_unit(d: Dictionary, team: int, facing: Vector2, pos: Vector2, unit_label: String) -> Unit:
	...
	# 4). Infantry carry a pilum beside the gladius they deploy holding, and cavalry carry a
	# spatha sidearm beside their deployed lance; a row without the key keeps Unit's 0 default,
	# which equip_weapon refuses -- so those units have no switch available.
	if d.has("sidearm"):
