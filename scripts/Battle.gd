...
const FogOverlayRef = preload("res://scripts/FogOverlay.gd")
...
# Distance in metres between the two spawn lines, for a caller that wants the battle to
# open wider (or narrower) than the map's own lines: _ready applies it through
# BattleMap.with_line_gap, which keeps team 0's line where it is, moves team 1's line to
# that distance below it, and grows the field with it so team 1 keeps its ground behind
# the line. <= 0 (the default) keeps `spawn_line_ys`/`field` exactly as set, so every
# existing battle, demo, and replay is unchanged. Settable BEFORE the node enters the
# tree, like the map fields above: a campaign clash fills it from CampaignBattle.pending's
# deployment_gap_m (the defended province's own value), the demo recorder from an input
# script's field of the same name. Authored in metres, like the loadout's reaches and
# gaits, and converted once at the spawn boundary in _ready.
var deployment_gap_m: float = -1.0
const TERRAIN_COLOR := {
...
## - OrderGuards.current_engaged_fraction() evaluates real per-soldier contact/weapon reach
##   against adjacent enemy soldiers. A light flank or corner graze where fewer than ~10% of
##   the unit's soldiers are in reach records a low peak fraction (< 0.10) and allows the
##   march to resume automatically without cancelling, whereas a heavier engagement (>= 0.10)
##   vets the destination on disengage.
##
## What ships:
##
## - The fight never crossed the threshold (e.g. a brief flank graze): the march resumes
##   automatically once disengaged.
## - It crossed the threshold, but the destination is still clear of any living enemy: the march
...
## - It crossed the threshold AND the destination now sits inside a living enemy's own footprint:
##   the order cancels outright, rather than blindly marching into ground the enemy now holds --
...
# like the alphas the design doc's Phase 2 parameter table calls out.
var fog_unexplored_color: Color = Color(0.05, 0.06, 0.05, 1.0)
var fog_explored_dim_alpha: float = 0.45
var _fog_overlay: Node2D = null
var _fog_grid_w: int = 0
var _fog_grid_h: int = 0
...
func _ready() -> void:
	...
	# Terrain fog overlay. Also always present; its grid (which needs the final `field`)
	# is sized further down once the map/campaign/replay blocks below have settled field.
	_fog_overlay = FogOverlayRef.new()
	_fog_overlay.name = "FogOverlay"
	_fog_overlay.unexplored_color = fog_unexplored_color
	_fog_overlay.explored_dim_alpha = fog_explored_dim_alpha
	...
	# The metres->world conversion happens once here, at the spawn boundary, like the
	# loadout's gait speeds and weapon reaches.
	if deployment_gap_m > 0.0 and Replay.mode != Replay.Mode.PLAYBACK:
		var widened: Dictionary = BattleMapRef.with_line_gap(
				deployment_gap_m * WorldScaleRef.WU_PER_M, field, spawn_line_ys)
		field = widened["field"]
	...
	# same guard sight_scale applies just above.
	if fog_cell <= 0.0 or not is_finite(fog_cell):
		fog_cell = DEFAULT_FOG_CELL
	_fog_grid_w = maxi(1, int(ceil(field.size.x / fog_cell)))
	_fog_grid_h = maxi(1, int(ceil(field.size.y / fog_cell)))
	_fog_explored = PackedByteArray()
	_fog_explored.resize(_fog_grid_w * _fog_grid_h)
	_fog_explored_remaining = _fog_grid_w * _fog_grid_h
	_fog_overlay.field = field
	_fog_overlay.cell_size = fog_cell
	_fog_overlay.grid_w = _fog_grid_w
	_fog_overlay.grid_h = _fog_grid_h
...
func capture_snapshot() -> Dictionary:
	...
	var snap: Dictionary = {
		...
		"fog_explored": _fog_explored.duplicate(),
		"fog_explored_remaining": _fog_explored_remaining,
	}
...
## The fog team's own units among `units` -- the observers whose perception both
## _tick_fog's enemy-visibility pass and the terrain-exploration pass below read from.
func _fog_observers() -> Array:
	var out: Array = []
	for u in _fog_units_in_play():
		if u.team == fog_team:
			out.append(u)
