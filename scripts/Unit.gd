...
## The weapon type currently HELD. Mutable from phase 4 on: equip_weapon() rewrites it
## (and the per-soldier ids, and the derived attack_range) when a switch order lands, so
## nothing may treat it as spawn-constant -- spawn_weapon_type_id below is what stays put.
var weapon_type_id: int = LoadoutRegistry.WEAPON_GLADIUS
## The weapon this regiment DEPLOYED holding, fixed for the unit's whole life even after a
## switch. Two readers need a weapon id that cannot move: SpawnFingerprint.record_of, whose
## digest is documented to hash spawn-time-stable fields only (a mid-battle switch would
## otherwise re-stamp a demo artifact's own fingerprint and fail its load check), and the
## weapon-switch toggle itself, which needs somewhere to switch BACK to once the sidearm is
## in hand.
var spawn_weapon_type_id: int = LoadoutRegistry.WEAPON_GLADIUS
## The OTHER weapon these soldiers carry -- the one a switch order swaps into and out of
## (docs/soldier-loadout-design.md phase 4): the legionary's pilum alongside his gladius.
## 0 is the registry's reserved-invalid id and means this regiment carries no second
## weapon at all, which is every roster type but Infantry today; equip_weapon() refuses
## an unresolvable id, so those units simply have no switch to make rather than needing a
## carries-a-sidearm branch of their own.
var sidearm_type_id: int = 0
...
# shield-wall-class defending soldier's own formation adds to SoldierEnemyContact's
# per-pair contact radius before an attacker counts as touching it (see
# formation_containment_margin below and docs/individual-collision-design.md's
# melee-intermixing-depth note). Scaled off soldier_body_radius() rather than a flat
# metre value so it stays proportionate to the body it protects. Kept small enough
# that even the widest same-type pairing (two infantry, both at this scale) stays
# well inside SoldierSpatialHash.CELL_SIZE's own margin over the raw body-radius floor
# it's pinned against (see that class's own invariant test) -- cavalry never
...
# see _formed_turn_gait_frac) at which the corner-slot derate reaches formed_turn_tracking_frac.
# Calibrated against this fix's own reference case, an 80-mount Cavalry block: pivot radius
# 505.9644 wu (pinned by test_deep_cavalry_formed_pivot_rate_bounded_for_tracking) over a
# marching band of file_pitch_wu() 40 wu x SoldierBodies.MARCHING_CORRIDOR_PROXIMITY_MULT
# 4.5 = 180 wu, giving 505.9644 / 180 = 2.810913. Below this depth ratio a formation's own
# footprint spans too few marching-bands for a lagging body to ever need diverting into a
# perimeter corridor (or blobbing) before it catches its moving slot, so no derate is needed
# there -- a plain Infantry line (depth ratio ~0.85-1.74 across a 30-120 soldier count) and a
# LOOSE Archers block (~1.49) both stay at frac 1.0, and only a genuinely deep/wide block
# like the issue's cavalry, or a tightly-packed 140-strong Spearmen block (~1.89), pays any
# of this fix's pacing cost -- and that Spearmen cost is a mild ~11% (frac ~0.89), not the
# flat 66.7% a single global constant applied to every formed pivot regardless of size.
...
## Equip `type_id` as this regiment's weapon, mid-battle (docs/soldier-loadout-design.md
## phase 4). Rewrites every place the equipped weapon is read from, so the whole sim
## follows the switch in the same tick instead of some consumers answering with the old
## type:
##   - _sim_soldier_weapon_id, the per-soldier array strike-time lethality resolves
##     through (soldier_lethality) -- the array phase 1 built for exactly this write;
##   - weapon_type_id, which melee_attack_interval()/weapon_rest_angle()/_foot_kind()
##     read, and -- the reason this one is not optional -- which SoldierBodies.seed, its
	...
	if enemy != null:
		...
		if in_contact and (target_enemy != null or not has_move_target or chasing \
			...
			if faced and _attack_cd <= 0.0:
				...
				# resolve_soldier_melee (via UnitCombat.strike's engaged branch) already fights
				# every adjacent engaged enemy simultaneously, per soldier -- no round-robin
				# needed to spread damage across a multi-attacker press; see that function's
				# own doc.
...
## FORMED_TURN_TRACKING_FRAC / FORMED_TURN_DEPTH_RATIO_REF's own doc comments for the
## calibration and the mechanism this derates against). Expresses the block's pivot
## radius as a DEPTH RATIO -- how many marching-corridor bands (file_pitch_wu() x
## SoldierBodies.MARCHING_CORRIDOR_PROXIMITY_MULT) wide the block's own footprint is --
## rather than reading pivot_radius as an absolute world-unit distance, since a raw wu
## comparison can't tell a Cavalry block's wide per-mount spacing from a tightly-packed
## Spearmen block's depth: the same absolute radius means a very different "how many
## bodies deep" for each. Below FORMED_TURN_DEPTH_RATIO_REF, a lagging body can always
## close its distance to a swinging slot within the corridor's own direct-arrival band
## before ever needing to divert around the formation's perimeter, so the pivot runs at
## full jog_speed pace; past it, the derate ramps down (inversely in the depth ratio)
## toward formed_turn_tracking_frac at the reference ratio itself, and further for an
## even deeper block -- reserving exactly the catch-up headroom a block that size needs,
## instead of taxing every formed pivot by the same flat amount regardless of size.
func _formed_turn_gait_frac() -> float:
	...
	if depth_ratio <= FORMED_TURN_DEPTH_RATIO_REF:
		return 1.0
	var safe_tracking_frac: float = clampf(
			formed_turn_tracking_frac, FORMED_TURN_TRACKING_FRAC_FLOOR, 1.0)
	return clampf(safe_tracking_frac * FORMED_TURN_DEPTH_RATIO_REF / depth_ratio,
			FORMED_TURN_TRACKING_FRAC_FLOOR, 1.0)
...
func _ensure_file_assignment(count: int, files: int) -> void:
	...
	# match. The index-order fill it falls back on is a placeholder for that tick, not an
	# answer, so the file count stays invalid and the next query deals again; committing it
	# would satisfy this function's own early-out forever after and freeze the placeholder.
	# With NO bodies at all -- a fresh spawn, a far-tier unit -- the index-order fill IS the
	# answer, because the bodies are built FROM these slots, so there is nothing to wait for
	# and it commits like any other. That is the whole job of the _sim_soldier_pos test below:
	# it separates 'too few bodies to deal from' from 'no bodies by design'.
...
## Extra world-unit "personal space" this regiment's formation contributes to the
...
func formation_containment_margin() -> float:
	if is_cavalry:
		return 0.0
	match formation_mode:
...
## Resolve a melee cadence per soldier against `enemy` AND every other enemy unit this
## regiment is genuinely adjacent to and engaged with right now -- not just `enemy` alone --
...
func resolve_soldier_melee(enemy: Unit) -> void:
	var defenders: Array[Unit] = [enemy]
	for other in _adjacent_engaged_enemy_units():
		if other != enemy and other.is_engaged() and not other._sim_soldier_pos.is_empty():
			defenders.append(other)
	SoldierMelee.resolve(self, defenders)
