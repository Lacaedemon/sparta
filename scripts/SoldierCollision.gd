...
## Returns [impulse_attacker, impulse_defender] (Vector2 pair) representing the momentum
## exchange: attacker recoils backward along the strike axis, defender is shoved forward.
## Splits the impulse inversely by effective mass (which includes bracing): lighter units
## recoil harder.
##
## Args:
...
static func bidirectional_impulse(
	...
	# Impulse splits inversely by effective masses: lighter units get more recoil.
	# J_a / m_d_eff = J_d / m_a_eff (inverse ratio by Newton's third law)
	# J_a = J_total * m_d_eff / (m_a_eff + m_d_eff)
	# J_d = J_total * m_a_eff / (m_a_eff + m_d_eff)
