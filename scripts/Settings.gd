...
# Per-soldier ID overlay: each soldier's array index drawn at its mark in the selected unit,
# visible only at figure LOD. Developer debug feature for tracking soldier identity through
# maneuvers and reforms. Default off. Paired with SPARTA_DEMO_STATE_FULL dumps for verification.
