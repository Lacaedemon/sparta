...
401s on workflow validation until merged; it runs after merge)." GitHub only lets the
review action validate against the workflow version already on the base branch, so a PR
changing that same file can't be validated pre-merge — the action detects this and defers
review to post-merge instead of failing or posting a stub. Don't treat this as a broken/stub
review needing a manual re-dispatch (per the existing stub-review handling above) — check
the run's own annotations (`gh run view <run-id>`) before assuming a self-review is missing
for the usual reasons. (`Lacaedemon/sparta` PR #1123, 2026-07-27.)
