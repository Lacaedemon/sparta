...
**Every PR requires a fresh demo.** The scripted-input recorder is the **standard**
demo path as of #318/#321. The older hand-authored/recorded `replay` path (documented
in the main `.claude/memories/sparta.md` "Authoring & verifying demo scenarios" section)
still works, but prefer freshly authored scripted input for anything that shows a
specific player gesture.
...
(IDLE/MOVING/FIGHTING/ROUTING/DEAD), `formation` (the serialized name strings
live in `DemoState.FORMATION_NAMES`, which must cover every `FORMATION_*` int
const on `scripts/Unit.gd` -- for valid tokens read that table, not a list
here), `soldiers`, `current_speed`,
