...
| `chars` | Flags curly quotes, en/em dashes, and the multiplication sign (U+00D7) in the Quarto docs (`*.qmd`, `*.R`), which are kept plain-ASCII. Remedy differs by file type: `x` or `*` in `.R`, `&times;` in `.qmd` prose (a `\uXXXX` escape is rendered literally by Pandoc, so it is not a substitute there). | `check-non-standard-chars.yml` |
