...
| `file_length` | Caps NEW `scripts/*.gd` files (added by this diff, not existing ones) at 100 lines (override with `SPARTA_CHECK_MAX_NEW_FILE_LINES`) — CLAUDE.md's "one function per file" convention as a hard budget for genuinely new files. Diff-scoped like `comments`, sharing its base resolution: a whole-tree gate would fail on the 167 of 274 tracked `*.gd` files that already exceed 100 lines. Scoped to `scripts/`, not `test/` (this repo's test files deliberately group many test functions per file). | `check-comment-citations.yml` |
